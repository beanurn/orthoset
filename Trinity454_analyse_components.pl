...
my $CDS_threshold = 24; # min. number of utr bases for which a search for obscured cds is initiated and min. # bases 
			...
			# as an obscured cds
my $TOTAL_FRAGMENTS = 34.4; # number of Bb Illumina fragments after QC (millions), data for the other taxa: 
				# Bvv: 41.0
				# Bo: 34.4
				# Bvs: 25.2
...
my $rsem;
my $rsem_line = '';
# my $rsemfile = '/tmp/Trinity_stuff_bnurnberger/bb_RSEM.isoforms.fpkm.sorted';   # will be opened at the end to add fpkm data to output file
...
# matching nucleotide codes
my %matchcodes = (
        A => ['A','N','M','R','W','H','V','D'],
        C => ['C','N','M','Y','S','H','B','V'],
        G => ['G','N','R','S','K','B','V','D'],
        T => ['T','N','W','Y','K','H','B','D'],
        N => ['A','C','T','G','N','M','R','W','Y','S','K','H','B','V','D'],
        M => ['A','C','N','M','R','W','Y','S','H','B','V','D'],
        R => ['A','G','N','M','R','W','S','K','H','B','V','D'],
        W => ['A','T','N','M','R','W','Y','K','H','B','V','D'],
        Y => ['C','T','N','M','Y','W','S','K','H','B','V','D'],
        S => ['C','G','N','M','R','Y','S','K','H','B','V','D'],
        K => ['T','G','N','R','W','Y','S','K','H','B','V','D'],
        B => ['C','T','G','N','M','R','W','Y','S','K','H','B','V','D'],
        D => ['A','T','G','N','M','R','W','Y','S','K','H','B','V','D'],
        H => ['A','C','T','N','M','R','W','Y','S','K','H','B','V','D'],
        V => ['A','C','G','N','M','R','W','Y','S','K','H','B','V','D'],
);

my $intron_info_file = 'var454_intron_info.txt';
my $iinfo;
my $intron_start;
my $intron_end;

my $fshiftfile = 'var454_frameshift_overlap.txt';
my $fshift;
...
# sequences sorted in numerial order of their Trinity index 
my @seqnames_sorted;
my @sequences_sorted;
...
#	print "CURRENT ALIGNMENT: $alignment\n";
...
#	$seqnames_sorted[1] =~ /comp\d+_c\d+/;
#	if (!($1 eq $rsem_component)) {
#		collect_coverage_info ($1);
#	}
	...
	while (my $line = <$in>) {
		...
		if ($line =~ />/) { 
			...
			if ($line =~ 'xt' or $line =~ 'orf') {
				...
				push (@seqnames_sorted,$line);
				if ($first) {$first = 0 }
				else {
					push (@sequences_sorted, $raw_seq);
					$raw_seq = '';
				}
			}
		}
...
#	print "number of blocks after record_pres_abs: ", scalar(@{$changepos{$i}}), "\n";
...
my @poorAlignFlag = ();
my @IdList = ();
my $poorAlignLength = 0;
my $poorAlignStart = 0;
my $current = 'good';   # current block status
my $goodSum;
my $goodCount;
my $poorSum;
my $poorCount;
...
#		print "component $component\n";
		...
		for ($k=0; $k<$alignment_length-$AL_WINDOW; $k++) {
			if (substr($aseq,$k,$AL_WINDOW) =~ /[ACTG]{$AL_WINDOW}/ and substr($bseq,$k,$AL_WINDOW) =~ /[ACTG]{$AL_WINDOW}/) {
				...
				if ($id < $ID_LOW) {
					if ($current eq 'good') {
						$poorAlignStart = scalar(@IdList);
						$poorBlocks++;
						$current = 'poor';
#						print "\nchanging from good to poor\n";
#						print "k = $k, currentArraySize = $poorAlignStart\n"; 

					}
					push @poorAlignFlag, 1;
				}
				else {
					if ($current eq 'poor') {
						$currentArraySize = scalar(@IdList);
						$currentPoorRange = $currentArraySize  - $poorAlignStart;
#						print "\nchanging from poor to good\n";
#						print "k = $k, currentArraySize = $currentArraySize\n"; 

						if ($currentPoorRange > $poorAlignLength) {
							for ($m=0;$m < $poorAlignStart;$m++) {
								$poorAlignFlag[$m] = 0;
							}
							$poorAlignLength = $currentPoorRange;
						}
						else {
							for ($m=$poorAlignStart;$m < $currentArraySize;$m++) {
								$poorAlignFlag[$m] = 0;
							}
						}
						$current = 'good';
					}
					push @poorAlignFlag, 0;
				}
			}   # two gap-less windows for which to compute ID
#			if (@IdList != @poorAlignFlag) {
#				print "k = $k, array sizes differ: IdList = ", scalar(@IdList), ", poorAlignFlag = ", scalar(@poorAlignFlag), "\n";
#				foreach $item (@IdList) { print "$item " }
#				print "\n\n";
#				foreach $item (@poorAlignFlag) { print "$item " }
#				print "\n\n";
#			}
		} # traversing the alignment one nucleotide at a time

#		print "\nfinished traversing alignment, poorBlocks = $poorBlocks\n";
		if ($current eq 'poor') {
			$currentArraySize = scalar(@IdList);
			$currentPoorRange = $currentArraySize  - $poorAlignStart;
			if ($currentPoorRange > $poorAlignLength) {
		...
		if ($goodCount > 0) 	{ $goodMean = $goodSum / $goodCount }
		else 			{ $goodMean = 0 }
		if ($poorCount > 0) 	{ $poorMean = $poorSum / $poorCount }
		else			{ $poorMean = 0 }				
		$seq_stats[$i]->{goodCount} = $goodCount;
		$seq_stats[$i]->{goodMean} = $goodMean;
		$seq_stats[$i]->{poorCount} = $poorCount;
		$seq_stats[$i]->{poorMean} = $poorMean;
		$seq_stats[$i]->{poorBlocks} = $poorBlocks;

		$seq_stats[$j]->{goodCount} = $goodCount;
		$seq_stats[$j]->{goodMean} = $goodMean;
		$seq_stats[$j]->{poorCount} = $poorCount;
		$seq_stats[$j]->{poorMean} = $poorMean;
		$seq_stats[$j]->{poorBlocks} = $poorBlocks;
...
my @fields;
	...
	while ($line = <$rsem>) {
		...
		if ($fields[0] =~ /$component_trin/) {
			...
			$seqno = $';
			$coverage{$seqno} = $fields[1] * $TOTAL_FRAGMENTS * $MEAN_READLENGTH / 1000;
#			print "$component_trin\t$seqno\t$coverage{$seqno}\n";
			$rsem_component = $component_trin;
			$found = 1;
		}
		elsif ($found)  { 
...
sub initialise_seq_stats {
	...
	for ($i=0; $i < $seq_count; $i++) {
		...
		for ($j=0; $j< $alignment_length; $j++) {
			if (substr($sequences_sorted[$i],$j,1) =~ /[ACTGN]/) { $seqbases++ }
		}
...
sub record_presence_absence  {
	...
	if (substr($sequences_sorted[$seq],0,1) =~ /[ACTGN]/) 	{ $current_pattern = 100 }
	else 						   	{ $current_pattern = 0 }

	if ($current_pattern == 100) { $start = 0 }
#	print "seq $seq, first base has pattern $current_pattern and start = $start\n";
	# loop over all remaining positions in the alignment and record changes of the pattern
	for ($j = 1; $j < $alignment_length; $j++) {	
		if (substr($sequences_sorted[$seq],$j,1) =~ /[ACTGN]/)	{ $pattern = 100 }
		else 							{ $pattern = 0 }
...
#			print "change of pattern at $j, new pattern = $pattern\n";
...
my $feature_end;
	...
	for my $href (@annotation)  {
		if ($seq == $$href{seqindex}) {      		# check if annotation belongs to current sequence
			$gapbases = 0;
			$start_block = -1;
			$end_block = -1;
...
#				print "seq $seq, $$href{match}, start: $$href{start}, end: $$href{end}\n";
...
#						print "looking for hrefstart: block $j, blockend = $block_end\n";
...
my $framepos;
	...
	if ($seq_stats[$t]->{orientation} eq '+') {
		...
		for ($i=0;$i<=$$orfend_ref;$i++) {            #  note that $$orfend_ref is the first base *after* the orf block
			if (substr($sequences_sorted[$t],$i,1) =~ /[ACTGN]/) {	$basecount += 1 }
	...
	if ($seq_stats[$seq]->{orientation} eq '+') {
		...
		for ($i=0;$i<=$$orfstart_ref;$i++) {
			if (substr($sequences_sorted[$seq],$i,1) =~ /[ACTGN]/) {	$basecount += 1 }
		}
...
my %comphash = (
        ...
        for ($i=length($seqstring)-1;$i>=0;$i--) {
                ...
                if (exists $comphash{$base})    {$revcomp .= $comphash{$base} }
                        ...
                        print "reverse_complement: comphash for key $base not found, $component\n"; 
                        $revcomp .= '-'; 
                }
        }
				...
				for ($j=0;$j<@{$patterns{$i}};$j++) {
					...
					if (!($blockend <= $gapstart or $blockstart >= $gapend) and ($status{$i}[$j] eq 'cds' or $status{$i}[$j] eq 'cporf')) {
						if (($gapend - $gapstart) % 3 != 0) {
							if ($seq_stats[$i]->{frameshift} > 0) { $splice_found = 1 }
						}
						else { $splice_found = 1 }
						last;
					}
...
my $query;
        ...
        for ($j=$k;$j<$k+$AL_WINDOW;$j++) {
                if (substr($sequences_sorted[$a],$j,1) eq substr($sequences_sorted[$b],$j,1)) { $matches++ }
                else {
                        $query = substr($sequences_sorted[$a],$j,1);
                        foreach $code (@{ $matchcodes{$query} }) {
                                if (substr($sequences_sorted[$b],$j,1) eq $code) {
                                        $matches++;
                                        last;
                                }
                        }
...
my $align_score = '';
	...
	$outname = $compname . '_' . $alignment . '_aligned.txt';
	open ($out,"> $outname") or die "Can't create $outname\n";
	
	print $out "454: $seqnames_sorted[0], trinity: $seqnames_sorted[1]\n\n";
	...
	for ($m=0;$m<$alignment_length;$m++) {
		...
		if ($a =~ /[ACTG]/ and $b =~ /[ACTG]/ and $a eq $b) { $matches .= '*' }    # THIS needs to be updated to allow for ambiguity codes
		else						{ $matches .= ' ' }
...
for ($i=0;$i<=$seq;$i++) {
	...
	print "seq $i, no of blocks: ", scalar @{$patterns{$i}},"\n";
	for ($j=0;$j<@{$patterns{$i}};$j++) {
...
my $outfilename = 'component_crunch_var454.txt';
...
if ($comp_count % 10 == 0) {
	print $out ">seq\t";
	print $out "align\tname\tlen\torient\torf\to_le\txt\txt_le\tcpo\tcpo_le\tstart\tstop\tintern\t";
	print $out "utr\tutr_le\tintr\tintr_le\tindl\tindl_le\tspl\tspl_le\tcds\tcds_le\tplain\tpln_le\t";
	print $out "good\tgdMean\tpoor\tpoor_le\tprMean\tfshft\n";
}
print $out ">$component\n";
for ($i=0;$i<$seq_count;$i++) {
	print $out "seq$i\t$alignment\t$seqnames_sorted[$i]\t$seq_stats[$i]->{basecount}\t$seq_stats[$i]->{orientation}\t$seq_stats[$i]->{orfcount}\t";
	...
	print $out "$seq_stats[$i]->{goodCount}\t";
	printf $out "%.2f\t",$seq_stats[$i]->{goodMean};
	print $out "$seq_stats[$i]->{poorBlocks}\t$seq_stats[$i]->{poorCount}\t";
	printf $out "%.2f\t",$seq_stats[$i]->{poorMean};
	print $out "$seq_stats[$i]->{frameshift}\n";
